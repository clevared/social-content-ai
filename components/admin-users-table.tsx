...
export function AdminUsersTable({ users: initial }: { users: User[] }) {
  ...
  const save = async (userId: string) => {
    ...
    if (res.ok) {
      setUsers((prev) =>
        prev.map((u) => (u.id === userId ? { ...u, ...editData } : u))
      )
      setEditing(null)
    }
  }
          ...
          <thead className="bg-white/5">
            <tr>
              {[
                ...
                <th
                  key={h}
                  className="text-left px-4 py-3 text-slate-400 text-xs font-medium"
                >
                  {h}
                </th>
              ))}
            ...
            {users.map((u) => (
              <tr key={u.id} className="hover:bg-white/5 transition-colors">
                ...
                <td className="px-4 py-3">
                  {editing === u.id ? (
                    <select
                      value={editData.plan || u.plan}
                      onChange={(e) =>
                        setEditData((d) => ({ ...d, plan: e.target.value }))
                      }
                      className="bg-slate-700 text-white text-xs rounded px-2 py-1"
                    >
                ...
                <td className="px-4 py-3">
                  {editing === u.id ? (
                    <input
                      ...
                      onChange={(e) =>
                        setEditData((d) => ({ ...d, credits: +e.target.value }))
                      }
